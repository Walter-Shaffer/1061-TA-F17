...
public class TemperatureConverter{
 ...
 public static void main(String[] args) {
	 ...
	 if (fahrenheit < MIN_FAHRENHEIT) { //The > was flipped the wrong direction, so I changed it to a <
	 ...
	 if (tempScale < 1 || tempScale > 4) { //The two comparative symbols were flipped, the < was suppposed to be >
		 System.out.println("Unknown temperature scale  -" +
		 					"  cannot do calculation. Bye");
		 System.exit(0); //Added System.exit(0)
	 }
	 else {
		 if (tempScale == 1) {
			 convertedDegrees = (fahrenheit - MIN_FAHRENHEIT)*5/9.0;//Switched the + to the - and added parenthesis
			 tempScaleStr="Kelvin"; 
		 } else if (tempScale == 2) { //Just took out the || tempScale == 3
 			convertedDegrees = fahrenheit - MIN_FAHRENHEIT;  
 			tempScaleStr="Rankine";
 			
		 } else if (tempScale == 3) { 
			 convertedDegrees = (fahrenheit - 32.0) * 4/9;//Added parenthesis
			 tempScaleStr="Reaumur";
				 
		 } else {
			 convertedDegrees = (fahrenheit - 32.0) * 5/9; //flipped the fraction to match the formula
